...
var resolveSCTPAddrTests = []resolveSCTPAddrTest{
	{"sctp", "127.0.0.1:0", &SCTPAddr{IP: IPv4(127, 0, 0, 1), Port: 0}, nil},
	{"sctp4", "127.0.0.1:65535", &SCTPAddr{IP: IPv4(127, 0, 0, 1), Port: 65535}, nil},

	{"sctp", "[::1]:0", &SCTPAddr{IP: ParseIP("::1"), Port: 0}, nil},
	{"sctp6", "[::1]:65535", &SCTPAddr{IP: ParseIP("::1"), Port: 65535}, nil},

	{"sctp", "[::1%en0]:1", &SCTPAddr{IP: ParseIP("::1"), Port: 1, Zone: "en0"}, nil},
	{"sctp6", "[::1%911]:2", &SCTPAddr{IP: ParseIP("::1"), Port: 2, Zone: "911"}, nil},

	{"", "127.0.0.1:0", &SCTPAddr{IP: IPv4(127, 0, 0, 1), Port: 0}, nil}, // Go 1.0 behavior
	{"", "[::1]:0", &SCTPAddr{IP: ParseIP("::1"), Port: 0}, nil},         // Go 1.0 behavior
...
func TestResolveSCTPAddr(t *testing.T) {
	...
	for i, tt := range resolveSCTPAddrTests {
		...
		rtaddr, err := ResolveSCTPAddr(addr.Network(), addr.String())
		if err != nil {
			t.Errorf("#%d: %v", i, err)
...
var sctpListenerNameTests = []struct {
	...
	{"sctp4", &SCTPAddr{IP: IPv4(127, 0, 0, 1)}},
	{"sctp4", &SCTPAddr{}},
	{"sctp4", nil},
}
...
func TestIPv6LinkLocalUnicastSCTP(t *testing.T) {
	testenv.MustHaveExternalNetwork(t)
	...
	for i, tt := range ipv6LinkLocalUnicastSCTPTests {
		...
		if la, ok := ln.Addr().(*SCTPAddr); !ok || !tt.nameLookup && la.Zone == "" {
		...
		defer c.Close()
		if la, ok := c.LocalAddr().(*SCTPAddr); !ok || !tt.nameLookup && la.Zone == "" {
			t.Fatalf("got %v; expected a proper address with zone identifier", la)
		}
		if ra, ok := c.RemoteAddr().(*SCTPAddr); !ok || !tt.nameLookup && ra.Zone == "" {
			t.Fatalf("got %v; expected a proper address with zone identifier", ra)
		}

		if _, err := c.Write([]byte("SCTP OVER IPV6 LINKLOCAL TEST")); err != nil {
			t.Fatal(err)
		}
		b := make([]byte, 32)
...
func TestSCTPSelfConnect(t *testing.T) {
	if runtime.GOOS == "windows" {
		// TODO(brainman): do not know why it hangs.
		t.Skip("known-broken test on windows")
	}

	ln, err := newLocalListener("sctp")
	if err != nil {
		t.Fatal(err)
	}
	var d Dialer
